...
static void gdt_set_entry(int index, uint32_t base, uint32_t limit, uint8_t access, uint8_t flags) {
    ...
    gdt[index].limit_row    =   limit & 0xFFFF;
...
static void tss_init(uint32_t kernel_stack) {
    memse(&tss, 0, sizeof tss);

    tss.ss0     =   GDT_KERNEL_DATA;
    tss.esp0    =   kernel_stacks;
    
    //no I/O perm bitmap: pointing past the end of the TSS denies rung 3 every port.
    tss.iomap_base = sizeof tss;

    gdt_set_entry(5, (uint32_t)&tss, siezof tss -1, 0x89, 0x00);
}
...
void gdt_init(void) {
    ...
    gdt_set_entry(3, 0, 0xFFFFF, 0xFA, 0xC0)    //kerlen code, ring 3
    gdt_set_entry(4, 0, 0xFFFFF, 0xF2, 0xC0)    //keneld data, ring 3
